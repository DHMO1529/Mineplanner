...
namespace UI{
namespace Shape{
ConeDesigner::ConeDesigner():
 DialogWindow(L"円錐",Colours::lightgrey,false)
,m_component()
,m_blockType()
,m_size()
,m_height()
...
void ConeDesigner::closeButtonPressed()
{
	this->setVisible(false);
}
void ConeDesigner::onMyButtonClicked(Widget::MyButton* button)
{
	if(button == &m_preview)
	{
		//プレビュー
		BlockSet blocks = MinePlanner::Shape::CreateCone(getDiameter(),getHeight(),getBlockID());
...
int ConeDesigner::getDiameter()const
{
	return m_size.getValue();
}
int ConeDesigner::getHeight()const
{
	return m_height.getValue();
}
int ConeDesigner::getBlockID()const
{
